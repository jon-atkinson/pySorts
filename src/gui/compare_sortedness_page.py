...
class CompareSortednessPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        left_panel.grid(row=0, column=0, sticky="esw")
        right_panel = ttk.Frame(self)
        right_panel.grid(row=0, column=1, sticky="esw")
        ...
        for language, algorithms in gui_config.config["algorithms"].items():
            for algorithm in algorithms:
                ...
                self.algorithms[key] = {"algorithm": algorithm, "language": language}
        self.selected_algorithm = tk.StringVar()
        row_num = 0
        for idx, key in enumerate(self.algorithms.keys()):
            input_radio_button = ttk.Radiobutton(
                left_panel,
                text=key,
            ...
            input_radio_button.grid(row=idx, column=0, sticky="w")
            row_num += 1
        ...
        for sortedness in self.sortedness_options.keys():
            ...
            checkbox = ttk.Checkbutton(
                left_panel, style="TCheckbutton", text=sortedness, variable=var
            )
        ...
        button_frame.grid(row=len(self.algorithms), column=0, sticky="w", pady=10)

        select_all_sortedness_button = ttk.Button(
            button_frame,
            text="Select All",
            command=self.select_all_sortedness,
            width=10,
        )
        select_all_sortedness_button.grid(row=0, column=0)
        deselect_all_sortedness_button = ttk.Button(
            button_frame,
            text="Deselect All",
            command=self.deselect_all_sortedness,
            width=10,
        )
        deselect_all_sortedness_button.grid(row=0, column=1)

        sliders_container = ttk.Frame(left_panel)
        sliders_container.grid(row=len(self.algorithms), column=1, sticky="s")

        self.sweep_start = ttk.Label(sliders_container, text="Start: 00001")
        self.sweep_start.grid(row=0, column=0, sticky="w")
        ...
        self.min_slider.grid(row=0, column=1, pady=2)
        self.min_sweep_subt_hundred_button = ttk.Button(
            sliders_container,
            text="-100",
            style="Increment.TButton",
            command=self.min_sweep_subt_hundred,
        )
        self.min_sweep_subt_hundred_button.grid(row=0, column=2, pady=2)
        self.min_sweep_subt_ten_button = ttk.Button(
            sliders_container,
            text="-10",
            style="Increment.TButton",
            command=self.min_sweep_subt_ten,
        )
        self.min_sweep_subt_ten_button.grid(row=0, column=3, pady=2)
        self.min_sweep_add_ten_button = ttk.Button(
            sliders_container,
            text="+10",
            style="Increment.TButton",
            command=self.min_sweep_add_ten,
        )
        self.min_sweep_add_ten_button.grid(row=0, column=4, pady=2)
        self.min_sweep_add_hundred_button = ttk.Button(
            sliders_container,
            text="+100",
            style="Increment.TButton",
            command=self.min_sweep_add_hundred,
        )
        self.min_sweep_add_hundred_button.grid(row=0, column=5, pady=2)

        self.sweep_end = ttk.Label(sliders_container, text="Stop: 00001")
        self.sweep_end.grid(row=1, column=0, sticky="w")
        ...
        self.max_slider.grid(row=1, column=1, pady=2)
        self.max_sweep_subt_hundred_button = ttk.Button(
            sliders_container,
            text="-100",
            style="Increment.TButton",
            command=self.max_sweep_subt_hundred,
        )
        self.max_sweep_subt_hundred_button.grid(row=1, column=2, pady=2)
        self.max_sweep_subt_ten_button = ttk.Button(
            sliders_container,
            text="-10",
            style="Increment.TButton",
            command=self.max_sweep_subt_ten,
        )
        self.max_sweep_subt_ten_button.grid(row=1, column=3, pady=2)
        self.max_sweep_add_ten_button = ttk.Button(
            sliders_container,
            text="+10",
            style="Increment.TButton",
            command=self.max_sweep_add_ten,
        )
        self.max_sweep_add_ten_button.grid(row=1, column=4, pady=2)
        self.max_sweep_add_hundred_button = ttk.Button(
            sliders_container,
            text="+100",
            style="Increment.TButton",
            command=self.max_sweep_add_hundred,
        )
        self.max_sweep_add_hundred_button.grid(row=1, column=5, pady=2)

        self.step = ttk.Label(sliders_container, text="Step: 0001")
        self.step.grid(row=2, column=0, sticky="w")
        ...
        self.step_slider.grid(row=2, column=1, pady=2)
        self.step_subt_hundred_button = ttk.Button(
            sliders_container,
            text="-100",
            style="Increment.TButton",
            command=self.step_subt_hundred,
        )
        self.step_subt_hundred_button.grid(row=2, column=2, pady=2)
        self.step_subt_ten_button = ttk.Button(
            sliders_container,
            text="-10",
            style="Increment.TButton",
            command=self.step_subt_ten,
        )
        self.step_subt_ten_button.grid(row=2, column=3, pady=2)
        self.step_add_ten_button = ttk.Button(
            sliders_container,
            text="+10",
            style="Increment.TButton",
            command=self.step_add_ten,
        )
        self.step_add_ten_button.grid(row=2, column=4, pady=2)
        self.step_add_hundred_button = ttk.Button(
            sliders_container,
            text="+100",
            style="Increment.TButton",
            command=self.step_add_hundred,
        )
        self.step_add_hundred_button.grid(row=2, column=5, pady=2)

        self.repeats = ttk.Label(sliders_container, text="Reps: 0001")
        self.repeats.grid(row=3, column=0, sticky="w")
        self.reps_val = tk.IntVar(value=1)
        self.reps_slider = ttk.Scale(
            sliders_container,
            from_=1,
            to=1000,
            orient="horizontal",
        ...
        self.reps_slider.grid(row=3, column=1, pady=2)
        self.reps_subt_hundred_button = ttk.Button(
            sliders_container,
            text="-100",
            style="Increment.TButton",
            command=self.reps_subt_hundred,
        )
        self.reps_subt_hundred_button.grid(row=3, column=2, pady=2)
        self.reps_subt_ten_button = ttk.Button(
            sliders_container,
            text="-10",
            style="Increment.TButton",
            command=self.reps_subt_ten,
        )
        self.reps_subt_ten_button.grid(row=3, column=3, pady=2)
        self.reps_add_ten_button = ttk.Button(
            sliders_container,
            text="+10",
            style="Increment.TButton",
            command=self.reps_add_ten,
        )
        self.reps_add_ten_button.grid(row=3, column=4, pady=2)
        self.reps_add_hundred_button = ttk.Button(
            sliders_container,
            text="+100",
            style="Increment.TButton",
            command=self.reps_add_hundred,
        )
        self.reps_add_hundred_button.grid(row=3, column=5, pady=2)
        ...
        self.plot_button.grid(row=1, column=0, padx=20, pady=20)

        lastGraphButton = ttk.Button(
            right_panel,
            text="Last Graph",
            style="Home.TButton",
            command=lambda: controller.show_frame("GraphPage"),
        )
        lastGraphButton.grid(row=2, column=0, pady=10)

        compare_sortedness_button = ttk.Button(
            right_panel,
            text="Feature Under Construction",
            style="Home.TButton",
            command=lambda: controller.show_frame("FeatureInProgressPage"),
        )
        compare_sortedness_button.grid(row=3, column=0, pady=10)
    ...
    def show_graph(self):
        ...
        n_steps, results = plot_sortedness_gui(
            alg_name, start, stop, step, arr_types, num_reps
        )
